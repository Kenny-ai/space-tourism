  ...
  return (
    <>
      <div className="flex items-center justify-between py-8 px-6 sm:py-0 sm:pr-0 lg:py-8 lg:pl-12">
        ...
        <div>
          {!showNav && <div className="sm:hidden cursor-pointer hover:scale-110">
            <Hamburger onClick={openNav} />
          </div>}

          <nav className="navbar hidden sm:flex bg-nav-bg text-body font-barlow text-[16px] p-10 gap-8 tracking-widest lg:px-30 xl:px-40 lg:gap-12">
            <NavLink to={"/"} className="uppercase">
              <span className="hidden lg:inline font-bold text-white">00</span>{" "}
              Home
            </NavLink>
            <NavLink to={"/destination"} className="uppercase">
              <span className="hidden lg:inline font-bold text-white">01</span>{" "}
              Destination
            </NavLink>
            <NavLink to={"/crew"} className="uppercase">
              <span className="hidden lg:inline font-bold text-white">02</span>{" "}
              Crew
            </NavLink>
            <NavLink to={"/technology"} className="uppercase">
